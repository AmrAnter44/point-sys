...
export async function POST(request: Request) {
  try {
    ...
    // البحث عن المستخدم بالإيميل أو الاسم من جدول User أو Staff
    const user = await prisma.user.findFirst({
      where: {
        OR: [
          { email: email },  // البحث بالبريد الإلكتروني
          { name: email },   // البحث بالاسم في جدول User
          { staff: { name: email } }  // ✅ البحث بالاسم في جدول Staff
        ]
      },
      include: {
        permissions: true,
