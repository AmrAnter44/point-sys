...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      nextNumber,
      currentNumber: counter.current
    })
  } catch (error) {
    console.error('Error fetching next receipt number:', error)
    return NextResponse.json({ nextNumber: 1001, currentNumber: 1000 })
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    return NextResponse.json({
      message: `تم تحديث إعدادات الإيصالات بنجاح`,
      nextNumber: startNumber
    })
  } catch (error) {
    console.error('Error updating receipt settings:', error)
    return NextResponse.json({ error: 'فشل تحديث إعدادات الإيصالات' }, { status: 500 })
  }
}
