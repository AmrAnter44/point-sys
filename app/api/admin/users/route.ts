...
export async function POST(request: Request) {
  try {
    ...
    // ✅ التحقق من اختيار موظف للكوتش أو الريسبشن
    if ((role === 'COACH' || role === 'STAFF') && !staffId) {
      return NextResponse.json(
        { error: role === 'COACH' ? 'يجب اختيار موظف لحساب الكوتش' : 'يجب اختيار موظف لحساب الريسبشن' },
        { status: 400 }
      )
    }
    ...
    if ((role === 'COACH' || role === 'STAFF') && staffId) {
      ...
      // ✅ التحقق من وظيفة الموظف
      if (role === 'STAFF' && staff.position !== 'ريسبشن') {
        return NextResponse.json(
          { error: 'يجب اختيار موظف من وظيفة ريسبشن فقط' },
          { status: 400 }
        )
      }
    }
    ...
    const user = await prisma.user.create({
      data: {
        ...
        role,
        isActive: true,
        staffId: (role === 'COACH' || role === 'STAFF') ? staffId : undefined  // ✅ ربط بالموظف للكوتش والريسبشن
      }
    })
