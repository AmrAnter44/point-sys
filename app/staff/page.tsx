...
export default function StaffPage() {
  ...
  const POSITIONS = [
    ...
    { value: 'تغذية', label: `🥗 ${t('positions.nutrition')}`, icon: '🥗' },
    { value: 'علاج طبيعي', label: `💆 ${t('positions.physiotherapy')}`, icon: '💆' },
...
const handleScan = async (staffCode: string) => {
  ...
  const getPositionColor = (position: string): string => {
    const colors: { [key: string]: string } = {
      ...
      تغذية: 'bg-lime-100 text-lime-800',
      'علاج طبيعي': 'bg-cyan-100 text-cyan-800',
    }
  ...
  return (
    <div className="container mx-auto px-4 py-6 md:px-6" dir={direction}>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold">
                {staff.reduce((sum, s) => sum + (s.salary || 0), 0).toFixed(0)} EGP
              </p>
            </div>
            <div className="text-5xl opacity-20">💰</div>
          </div>
      ...
      {/* Quick Access Buttons - Sales & Attendance */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <button
          onClick={() => router.push('/sales/commissions')}
          className="bg-gradient-to-br from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white rounded-xl p-6 shadow-lg transition-all transform hover:scale-105 active:scale-95"
        >
          <div className="flex items-center justify-between">
            <div className="text-left">
              <div className="text-5xl mb-3">💵</div>
              <h3 className="text-xl font-bold mb-1">{t('nav.salesCommissions')}</h3>
              <p className="text-emerald-100 text-sm">{t('staff.quickAccess.salesCommissionsDesc')}</p>
            </div>
        ...
        <button
          ...
          className="bg-gradient-to-br from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 text-white rounded-xl p-6 shadow-lg transition-all transform hover:scale-105 active:scale-95"
        >
          <div className="flex items-center justify-between">
            <div className="text-left">
              <div className="text-5xl mb-3">📊</div>
              <h3 className="text-xl font-bold mb-1">{t('nav.salesReport')}</h3>
              <p className="text-indigo-100 text-sm">{t('staff.quickAccess.salesReportDesc')}</p>
            </div>
            <div className="text-4xl opacity-30">→</div>
          </div>
        </button>
        ...
        <button
          ...
          className="bg-gradient-to-br from-cyan-500 to-cyan-600 hover:from-cyan-600 hover:to-cyan-700 text-white rounded-xl p-6 shadow-lg transition-all transform hover:scale-105 active:scale-95"
        >
          <div className="flex items-center justify-between">
            <div className="text-left">
              <div className="text-5xl mb-3">📋</div>
              <h3 className="text-xl font-bold mb-1">{t('nav.staffAttendance')}</h3>
              <p className="text-cyan-100 text-sm">{t('staff.quickAccess.staffAttendanceDesc')}</p>
            </div>
            <div className="text-4xl opacity-30">→</div>
          </div>
        </button>
                ...
                <div className="p-4 space-y-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-green-600">
                      {staffMember.salary ? `${staffMember.salary} EGP` : '-'}
                    </span>
                  </div>
