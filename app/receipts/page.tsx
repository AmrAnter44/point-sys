...
export default function ReceiptsPage() {
  ...
  const [receiptToCancel, setReceiptToCancel] = useState<Receipt | null>(null)
  ...
  const handleCancelReceipt = (receipt: Receipt) => {
    ...
    if (receipt.isCancelled) {
      setMessage('❌ هذا الإيصال ملغي بالفعل')
      setTimeout(() => setMessage(''), 3000)
      return
    }
  ...
  const confirmCancelReceipt = async () => {
    ...
    try {
      const response = await fetch('/api/receipts/cancel', {
        ...
        body: JSON.stringify({
          receiptId: receiptToCancel.id,
          reason: cancellationReason || 'لا يوجد سبب محدد'
        })
      })
    ...
    <div className="container mx-auto px-4 py-6 md:px-6" dir={direction}>
          ...
          <div className="mt-4 pt-4 border-t flex gap-3 items-end">
            <div className="flex-1">
              ...
              <input
                ...
                className="w-full px-3 py-2 border-2 rounded-lg text-sm focus:ring-2 focus:ring-orange-500"
                placeholder="1000"
              />
            </div>
            <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 md:px-4 border-2 rounded-lg focus:ring-2 focus:ring-orange-500"
            />
          </div>
        ...
        <div className="md:hidden space-y-4 mb-6">
          {currentReceipts.map((receipt) => {
            ...
            return (
              <div
                key={receipt.id}
                className={`rounded-lg shadow-lg p-5 border-r-4 ${
                  receipt.isCancelled
              ...
              >
                ...
                <div className="flex justify-between items-start mb-4 pb-3 border-b-2 border-gray-100">
                  <div>
                    ...
                    <span className={`font-bold text-xl ${receipt.isCancelled ? 'text-red-600 line-through' : 'text-orange-600'}`}>
                      #{receipt.receiptNumber}
                    </span>
                  </div>
                ...
                {receipt.isCancelled && (
                  <div className="bg-red-100 border-2 border-red-300 rounded-lg p-3 mb-4">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <p className="text-sm font-bold text-red-800">هذا الإيصال ملغي</p>
                    </div>
                    {receipt.cancelledBy && (
                      <p className="text-xs text-red-700">بواسطة: {receipt.cancelledBy}</p>
                    )}
                    {receipt.cancelledAt && (
                      <p className="text-xs text-red-700">
                        التاريخ: {new Date(receipt.cancelledAt).toLocaleString('ar-EG', {
                          year: 'numeric',
                          month: 'short',
                          day: 'numeric',
                          hour: '2-digit',
                    ...
                    {receipt.cancellationReason && (
                      <p className="text-xs text-red-700 mt-1">السبب: {receipt.cancellationReason}</p>
                    )}
                  </div>
                )}
                ...
                {receipt.type === 'Points Redemption' && details && (
                  <div className="bg-gradient-to-r from-yellow-50 to-orange-50 rounded-lg p-3 mb-4 border-2 border-yellow-300">
                    <div className="flex items-center gap-2 mb-2">
                      <span className="text-yellow-600 text-lg">🎁</span>
                      <p className="text-xs text-yellow-700 font-semibold">تفاصيل استبدال النقاط</p>
                    </div>
                    <div className="space-y-2">
                      <p className="text-sm text-yellow-800">
                        💎 استبدال <span className="font-bold text-orange-600">{details.pointsUsed?.toLocaleString()}</span> نقطة
                      </p>
                      <p className="text-sm text-yellow-700">
                        ↳ <span className="font-bold text-green-600">{details.rewardName}</span>
                      </p>
                      {details.daysAdded && (
                        <p className="text-xs text-yellow-600">
                          📅 تم إضافة <span className="font-semibold">{details.daysAdded} يوم</span> للعضوية
                        </p>
                      )}
                    </div>
                  </div>
                ...
                <div className="flex gap-2 mt-4 pt-3 border-t border-gray-200">
                  ...
                  <button
                    ...
                    className="flex-1 bg-orange-600 text-white px-3 py-2 rounded-lg hover:bg-orange-700 text-sm transition shadow-md font-semibold"
                  ...
                  {canEdit && !receipt.isCancelled && (
                    <button
                      ...
                      className="bg-yellow-600 text-white px-3 py-2 rounded-lg hover:bg-yellow-700 text-sm transition shadow-md"
                      title="إلغاء الإيصال"
                    >
                      ❌ إلغاء
                    </button>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                ...
                return (
                  <tr key={receipt.id} className={`border-t transition ${
                    receipt.isCancelled
                    ...
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-2">
                        <span className={`font-bold text-lg ${receipt.isCancelled ? 'text-red-600 line-through' : 'text-orange-600'}`}>
                          #{receipt.receiptNumber}
                        </span>
                        {receipt.isCancelled && (
                          <span
                            className="text-xs bg-red-600 text-white px-2 py-0.5 rounded-full font-bold cursor-help"
                            title={`ملغي بواسطة: ${receipt.cancelledBy || '-'}\nالتاريخ: ${receipt.cancelledAt ? new Date(receipt.cancelledAt).toLocaleString('ar-EG') : '-'}\nالسبب: ${receipt.cancellationReason || '-'}`}
                          >
                            ملغي
                          </span>
                        )}
                      </div>
                      {receipt.isCancelled && receipt.cancellationReason && (
                        <div className="mt-1 text-xs text-red-700 bg-red-100 px-2 py-1 rounded">
                          <strong>السبب:</strong> {receipt.cancellationReason}
                        </div>
                      )}
                    </td>
                    <td className="px-4 py-4">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-6 px-4 py-3 bg-gray-50 rounded-lg">
            ...
            <div className="flex items-center gap-2 text-sm">
              ...
              <select
                ...
                className="border border-gray-300 rounded-lg px-3 py-1 focus:ring-2 focus:ring-orange-500 focus:border-transparent"
              >
      ...
      {showEditModal && editingReceipt && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-5xl w-full max-h-[90vh] overflow-y-auto p-4" dir={direction}>
            ...
            <div className={`bg-orange-50 ${direction === 'rtl' ? 'border-r-4' : 'border-l-4'} border-orange-500 rounded-lg p-4 mb-6`}>
            ...
            <div className="flex gap-3 mt-6">
              <button
                ...
                className="flex-1 bg-orange-600 text-white py-3 rounded-lg hover:bg-orange-700 transition font-bold shadow-lg hover:shadow-xl"
              >
      ...
      {showCancelModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" dir={direction}>
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6">
            {/* Header */}
            <div className="flex items-center gap-3 mb-4 pb-4 border-b-2 border-red-200">
              <div className="bg-red-100 p-3 rounded-full">
                <span className="text-3xl">⚠️</span>
              </div>
              <div>
                <h3 className="text-2xl font-bold text-red-800">إلغاء الإيصال</h3>
                <p className="text-sm text-red-600">إيصال رقم #{receiptToCancel?.receiptNumber}</p>
              </div>
            </div>
            ...
            <div className="bg-yellow-50 border-2 border-yellow-300 rounded-lg p-4 mb-4">
              <p className="text-sm text-yellow-800 font-semibold">
                ⚠️ تحذير: عند إلغاء الإيصال سيتم:
              </p>
              <ul className="text-xs text-yellow-700 mt-2 space-y-1 mr-4">
                <li>• تسجيل مصروف تلقائياً بنفس مبلغ الإيصال</li>
                <li>• لن تتمكن من تعديل الإيصال بعد الإلغاء</li>
                <li>• سيظهر الإيصال بخلفية حمراء</li>
              </ul>
            </div>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 bg-gray-200 text-gray-800 px-6 py-3 rounded-lg hover:bg-gray-300 font-semibold transition"
              >
                إلغاء الأمر
              </button>
              <button
                ...
                className="flex-1 bg-red-600 text-white px-6 py-3 rounded-lg hover:bg-red-700 font-semibold transition disabled:bg-gray-400 disabled:cursor-not-allowed"
              >
                ✓ تأكيد الإلغاء
              </button>
            </div>
